...
const PreviewPlanSelectPage: React.FC = () => {
  ...
  const { mutateAsync: fetchLightPlansAsync } = useGptTrialLight();
  const { mutateAsync: fetchModeratePlansAsync } = useGptTrialModerate();
  const { mutateAsync: fetchStrongPlansAsync } = useGptTrialStrong();
